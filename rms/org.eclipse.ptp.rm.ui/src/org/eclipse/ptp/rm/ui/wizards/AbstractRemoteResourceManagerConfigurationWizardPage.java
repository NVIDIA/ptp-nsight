...
public abstract class AbstractRemoteResourceManagerConfigurationWizardPage extends AbstractConfigurationWizardPage {
	...
	private void initializeRemoteServices() {
		if (fRemoteServices == null) {
				...
				public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					...
					try {
						for (int i = 0; i < services.length; i++) {
							...
							progress.setTaskName(NLS.bind(Messages.AbstractRemoteResourceManagerConfigurationWizardPage_7,
									service.getName()));
			...
			fRemoteServices = initializedServices.toArray(new IRemoteServices[0]);
	 ...
	 * 
	 * @param id
	 *            id of remote services to select as default, or null to select
	 *            the system default
	 * @return id of the default services
	 */
