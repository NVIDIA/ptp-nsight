...
from  ptplib   import  PTPProxy, ptp_print, to_ptp_string, from_ptp_string, \
                       RTEV_OK, RTEV_NEWJOB, RTEV_NATTR, \
                       RTEV_PATTR, RTEV_JOBSTATE, RTEV_PROCOUT, \
                       RTEV_ATTRIB_MACHINEID, RTEV_ATTRIB_NODE_NAME, \
                       RTEV_ATTRIB_NODE_NUMBER, RTEV_ATTRIB_NODE_STATE, \
                       RTEV_ATTRIB_NODE_GROUP, RTEV_ATTRIB_NODE_USER, \
                       RTEV_ATTRIB_NODE_MODE, JOB_STATE_INIT, \
                       JOB_STATE_RUNNING, JOB_STATE_TERMINATED, JOB_STATE_ERROR, \
...
def discover(args):
    ...
    node_attr = [RTEV_ATTRIB_MACHINEID + '=' + '0']
    node_number = 0
    done = 0
    while not done:
    	...
    	if msg['cmd'] == 'mpdtrace_info':
    		node_attr.append(RTEV_ATTRIB_NODE_NUMBER + '=' + str(node_number))
    		node_number += 1
    		pos = msg['id'].rfind('_')
    		node_attr.append(RTEV_ATTRIB_NODE_NAME + '=' + str(msg['id'][:pos]))  # strip off port
    		node_attr.append(RTEV_ATTRIB_NODE_USER + '=' + mpd_get_my_username())
        	group = pwd.getpwnam(mpd_get_my_username())[3]
        	group_name = grp.getgrgid(group)[0]
     		node_attr.append(RTEV_ATTRIB_NODE_GROUP + '=' + group_name)
     		node_attr.append(RTEV_ATTRIB_NODE_STATE + '=' + 'up')
    		node_attr.append(RTEV_ATTRIB_NODE_MODE + '=' + '72')
...
def listprocs(args):
    ...
    ptp_jobid = int(args[0])
    if ptp_jobid >= len(jobIDMap):
    	return (RTEV_ERROR_PROCS, 'No such job')
    jobid = jobIDMap[ptp_jobid]
    conSock = open_mpd_console()
    if conSock == 0:
...
def procattrs(args):
    ...
    for v1 in pattr:
    	for v2 in v1:
    	    attrs.append(v2)
    extraEvent = (RTEV_JOBSTATE, [ptp_jobid, 4])
...
def kill(args):
    ...
    extraEvent = (RTEV_JOBSTATE, [ptp_jobid, JOB_STATE_TERMINATED])
    return (RTEV_OK, [])

def run(args):
    global parmdb, sigOccurred, streamHandler, manSocks
    global tmpJobID, runningJobs, jobIDMap, ringInfo, extraEvent
    ...
    while pos < len(args):
    	if args[pos] == 'jobID':
    	    ptp_jobid = int(args[pos+1])
    	elif args[pos] == 'execName':
    	    pgm_name = args[pos+1]
    ...
    msgToMPD['execs'][(0,nprocs-1)]   = exec_path + '/' + pgm_name
    ...
    if (msg['cmd'] == 'job_started'):
    	...
    	jobIDMap.insert(ptp_jobid, jobid)
    	debug_print('mpiexec: job %s started' % jobid)
    else:
    	return (RTEV_ERROR_RUN, 'Unknown msg: %s' % (msg) )

    streamHandler.set_handler(manSock,handle_man_input,args=(streamHandler,jobid,ptp_jobid))
    ...
    streamHandler.set_handler(manCliStdoutSock,
			  ...
			  args=(streamHandler,jobid,ptp_jobid))
    ...
    extraEvent = (RTEV_JOBSTATE, [ptp_jobid, JOB_STATE_RUNNING])
    return (RTEV_JOBSTATE, [ptp_jobid, JOB_STATE_INIT])

def handle_man_input(sock,streamHandler,jobid,ptp_jobid):
    global manSocks, ptpProxy
    ...
    if runningJobs[jobid] <= 0:
        if ptpProxy != None:
            ptpProxy.send_ok_event(RTEV_JOBSTATE, [ptp_jobid, job_state])
    	streamHandler.del_handler(sock)
    	manSocks.remove(sock)
    	sock.close()

def handle_cli_stdout_input(sock,streamHandler,jobid,ptp_jobid):
    global ptpProxy
    ...
    if not msg:
    	...
    	try:
    	    ...
    	    rank = 0
    	if ptpProxy != None:
    	    ptpProxy.send_ok_event(RTEV_PROCOUT, [ptp_jobid, rank, rest])
    	else:
    	    sys.stdout.write(msg)
    	    sys.stdout.flush()

def handle_cli_stderr_input(sock,streamHandler,jobid,ptp_jobid):
    msg = sock.recv(1024)
    if not msg:
        streamHandler.del_handler(sock)
    else:
