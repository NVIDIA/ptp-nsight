...
public abstract class AbstractProxyRuntimeClient extends AbstractProxyClient
	 ...
	 */
	private final LinkedBlockingQueue<IProxyEvent> events = new LinkedBlockingQueue<IProxyEvent>();
	/*
	 * Listener list for events generated by us
	 */
	private final Collection<IProxyRuntimeEventListener> listeners = Collections
	...
	public void filterEvents(String[] attrs) throws IOException {
		if (state != ProxyState.RUNNING) {
			throw new IOException(Messages.getString("AbstractProxyRuntimeClient_0")); ////$NON-NLS-1$
		}
		...
		while (state != ProxyState.IDLE && state != ProxyState.ERROR) {
			...
			case STARTUP:
				...
				if (event instanceof IProxyConnectedEvent) {
					...
					state = ProxyState.IDLE;
					fireProxyRuntimeStartupErrorEvent(eventFactory
							.newProxyRuntimeStartupErrorEvent(Messages.getString("AbstractProxyRuntimeClient_1")));
				} else if (event instanceof IProxyDisconnectedEvent) {
					state = ProxyState.IDLE;
					fireProxyRuntimeStartupErrorEvent(eventFactory
							.newProxyRuntimeStartupErrorEvent(Messages.getString("AbstractProxyRuntimeClient_2")));
				} else {
					state = ProxyState.ERROR;
					fireProxyRuntimeErrorStateEvent(eventFactory
							.newProxyRuntimeErrorStateEvent());
					throw new IllegalStateException(NLS.bind(
							Messages.getString("AbstractProxyRuntimeClient_3"), event
									.toString()));
				}
				break;
			...
			case INIT:
				...
				if (event instanceof IProxyDisconnectedEvent) {
					state = ProxyState.IDLE;
					fireProxyRuntimeStartupErrorEvent(eventFactory
							.newProxyRuntimeStartupErrorEvent(Messages.getString("AbstractProxyRuntimeClient_2"))); //$NON-NLS-1$
					...
					if (command != null) {
						...
						if (event instanceof IProxyOKEvent) {
							...
							fireProxyRuntimeErrorStateEvent(eventFactory
									.newProxyRuntimeErrorStateEvent());
							throw new IllegalStateException(NLS.bind(
									Messages.getString("AbstractProxyRuntimeClient_4"),
									event.toString()));
						}
					} else {
						state = ProxyState.ERROR;
						fireProxyRuntimeErrorStateEvent(eventFactory
								.newProxyRuntimeErrorStateEvent());
						throw new IllegalStateException(NLS.bind(
								Messages.getString("AbstractProxyRuntimeClient_5"), event
										.toString()));
					}
				}
				break;
					...
					if (command != null) {
						if (event instanceof IProxyOKEvent) {
							...
							fireProxyRuntimeErrorStateEvent(eventFactory
									.newProxyRuntimeErrorStateEvent());
							throw new IllegalStateException(NLS.bind(
									Messages.getString("AbstractProxyRuntimeClient_6"),
									event.toString()));
						}
					} else {
						state = ProxyState.ERROR;
						fireProxyRuntimeErrorStateEvent(eventFactory
								.newProxyRuntimeErrorStateEvent());
						throw new IllegalStateException(NLS.bind(
								Messages.getString("AbstractProxyRuntimeClient_7"), event
										.toString()));
					}
				}
				break;
				...
				if (event instanceof IProxyDisconnectedEvent) {
					...
					fireProxyRuntimeMessageEvent(eventFactory
							.newProxyRuntimeMessageEvent(Level.FATAL,
									Messages.getString("AbstractProxyRuntimeClient_2"))); //$NON-NLS-1$
				} else {
					command = getCommandForEvent(event);
					if (command != null) {
						...
						state = ProxyState.ERROR;
						throw new IllegalStateException(NLS.bind(
								Messages.getString("AbstractProxyRuntimeClient_8"), event
										.toString()));
					}
				}
				break;
			...
			default:
				throw new IllegalStateException(NLS
						.bind(Messages.getString("AbstractProxyRuntimeClient_9"), state
								.toString()));
			}
		}
	}
